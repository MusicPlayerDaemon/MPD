...
playlist_open_path(const char *path_fs)
{
	...
	if (playlist == NULL)
		playlist = playlist_list_open_path(path_fs);
...
playlist_open_in_playlist_dir(const char *uri)
{
	...
	struct playlist_provider *playlist = playlist_open_path(path_fs);
	g_free(path_fs);
 ...
 */
static struct playlist_provider *
playlist_open_in_music_dir(const char *uri)
{
	char *path_fs;
...
playlist_mapper_open(const char *uri)
{
	...
	if (spl_valid_name(uri)) {
		playlist = playlist_open_in_playlist_dir(uri);
		if (playlist != NULL)
			return playlist;
	}

	if (uri_safe_local(uri)) {
		playlist = playlist_open_in_music_dir(uri);
		if (playlist != NULL)
			return playlist;
	}
