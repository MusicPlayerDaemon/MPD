...
nfs_set_base(const char *server, const char *export_name) noexcept
{
	...
	const size_t server_length = std::strlen(server);
	const size_t export_name_length = std::strlen(export_name);

	if (server_length >= sizeof(nfs_base_server) ||
	    export_name_length > sizeof(nfs_base_export_name))
		return;

	std::memcpy(nfs_base_server, server, server_length + 1);
	std::memcpy(nfs_base_export_name, export_name, export_name_length);
...
nfs_check_base(const char *server, const char *path) noexcept
{
	...
	return std::strcmp(nfs_base_server, server) == 0 &&
	    std::memcmp(nfs_base_export_name, path,
